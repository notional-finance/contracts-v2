...
library ExchangeRate {
    ...
    uint256 internal constant ETH_RATE_STORAGE_SLOT = 1;
    ...
    function convertToETH(ETHRate memory er, int256 balance) internal pure returns (int256) {
        ...
        int256 result =
            balance.mul(er.rate).mul(multiplier).div(MULTIPLIER_DECIMALS).div(er.rateDecimals);
    ...
    function buildExchangeRate(uint256 currencyId) internal view returns (ETHRate memory) {
        ...
        if (currencyId == ETH) {
            ...
            if (
                bytes1(data << 80) != 0x00 /* mustInvert */
