...
contract NotionalV2FlashLiquidator is IFlashLoanReceiver {
    ...
    function executeOperation(
        ...
        // todo: if has transfer fees then we need to deposit first...
        ...
        if (action == 0x01) {
            ...
            (int256 localAssetCash, int256 netNTokens) = NotionalV2.liquidateLocalCurrency(
                liquidateAccount,
                localCurrency,
    ...
    function _redeemNToken(uint256 nTokenCurrencyId, uint96 nTokenBalance) internal returns (int256) {
        return
