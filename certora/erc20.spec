...
// rule basic_delegation_effect_delegateBySig(address delegator, address delegatee) {
// 	env e;
...
// 	assert getDelegateeByType(delegator, type) == delegatee;
// }
delegate_AtoBtoC(){
	address A;
	address B;
...
cannotApproveNonZeroWhenCurrentlyNonZero(env e, address spender, uint256 value)//Gadi - why this makes sense?
...
good_description "Checked implementation for transfer always returns `true` when not reverting."
{
	...
	address dst;
	uint256 amount;
	bool ret = sinvoke transfer(e, dst, amount);
...
good_description "Checked implementation for transferFrom always returns `true` when not reverting."
{
	...
	bool ret = sinvoke transferFrom(e, src, dst, amount);
...
good_description "An invocation of transfer may only increase recipient's balance and decrease sender's balance." 
{
...
description "transferFrom (from = $from, to = $to, by ${e.msg.sender}, value = $value) did not update participants balances and allowances as expected."
{
	...
	require origAllowance != 2^96;
	bool precondition = origBalanceOfFrom >= value && (caller == from || origAllowance >= value);
...
description "transferFrom (from = $from, to = $to, by ${e.msg.sender}, value = $value) did not update participants balances and allowances as expected."
{
	...
	bool precondition = origBalanceOfFrom >= value && (from == caller || origAllowance >= value);
	...
	// assert newFromBalanceChanged && newToBalanceChanged && newAllowanceChanged; //Gadi
}
...
Consider checking balance of x=$x or allowance from y=$y to z=$z."
{
	...
	env e2;
	require e2.block.number >= e.block.number;
	require e.msg.sender != 0 && e2.msg.sender != 0 && spender != 0 && value > 0;
...
description "Unallowed approval (increase of allowances) for $a"
{
	...
	env e1;
	uint256 newAllowance = sinvoke allowance(e1, _owner, spender);
...
description "allowance function may revert"
{
	...
	require e.msg.value == 0;
	invoke allowance(e, account, spender);
