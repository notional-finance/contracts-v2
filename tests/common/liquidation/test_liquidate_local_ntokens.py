...
def liquidation(
    ...
    liquidateOverride.setETHRateMapping(1, get_eth_rate_mapping(ethAggregators[0], discount=104))
...
def test_liquidate_ntoken_no_limit(liquidation, accounts):
...
def test_liquidate_ntoken_more_than_limit(liquidation, accounts):
    (liquidateOverride, liquidateSetup) = liquidation
    (cashGroup, markets) = liquidateOverride.buildCashGroupView(1)
...
def test_liquidate_ntoken_limit_to_user_specification(liquidation, accounts):
    (liquidateOverride, liquidateSetup) = liquidation
    (cashGroup, markets) = liquidateOverride.buildCashGroupView(1)
