...
def nToken(MockNTokenHandler, accounts):
    return accounts[0].deploy(MockNTokenHandler)


@given(currencyId=strategy("uint16"), tokenAddress=strategy("address"))
def test_set_perpetual_token_setters(nToken, currencyId, tokenAddress):
...
def test_deposit_parameters_failures(nToken):
...
def test_deposit_parameters(nToken, maxMarketIndex):
    currencyId = 1
...
def test_init_parameters_failures(nToken):
...
def test_init_parameters_values(nToken, maxMarketIndex):
    currencyId = 1
