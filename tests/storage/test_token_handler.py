...
def test_token_transfers(tokenHandler, MockERC20, accounts, decimals, fee):
...
def test_ctoken_mint_redeem(tokenHandler, accounts):
    env = TestEnvironment(accounts[0])
    env.enableCurrency("DAI", CurrencyDefaults)

    tokenHandler.setCurrencyMapping(1, False, (env.cToken["ETH"].address, False, TokenType["cETH"]))
    tokenHandler.setCurrencyMapping(
