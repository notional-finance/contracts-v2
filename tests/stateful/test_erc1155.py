...
def test_calldata_encoding_failure(environment, accounts):
    markets = environment.notional.getActiveMarkets(2)
    erc1155id = environment.notional.encodeToId(2, markets[0][1], 1)
    environment.notional.batchBalanceAndTradeAction(accounts[1], [
        get_balance_trade_action(
            2,
            "DepositUnderlying",
            [
                {"tradeActionType": "Lend", "marketIndex": 1, "notional": 100e8, "minSlippage": 0},
                {"tradeActionType": "Borrow", "marketIndex": 2, "notional": 50e8, "maxSlippage": 0}
            ],
...
def test_bidirectional_fcash_transfer_free_collateral(environment, accounts):
    ...
    erc1155id = environment.notional.encodeToId(2, markets[0][1], 1)
    amount = (Wei(2) ** 256) - Wei(50e8)
    # Account 1 is sending 50 fDAI to account[0], will trip free collateral
    environment.notional.batchBalanceAndTradeAction(accounts[1], [
        get_balance_trade_action(
            2,
            "DepositUnderlying",
...
def test_bidirectional_fcash_transfer(environment, accounts):
    ...
    environment.notional.batchBalanceAndTradeAction(accounts[0], [
        get_balance_trade_action(
            2,
            "DepositUnderlying",
            [
    ...
    assert len(toAssets) == 1
    assert len(fromAssets) == 1
    assert environment.notional.signedBalanceOf(accounts[0], erc1155ids[0]) == 0
...
def test_bidirectional_fcash_transfer_to_account_will_trade(
    ...
    data = web3.eth.contract(abi=environment.notional.abi).encodeABI(
        ...
        args=[
            accounts[1].address,  # Using "to" address
            [get_balance_action(2, "DepositUnderlying", depositActionAmount=500e18)],
        ],
    )
...
def test_transfer_and_batch_lend(environment, accounts):
    ...
    assert fromAssets[0][0] == 2
    assert fromAssets[0][1] == markets[0][1]
    assert fromAssets[0][2] == 1
    assert fromAssets[0][3] == 100e8
...
def test_batch_transfer_and_batch_lend(environment, accounts):
    ...
    # First lend fDAI to ensure sufficient balance
    environment.notional.batchLend(accounts[1], [action], {"from": accounts[1]})
    ...
    assert len(toAssets) == 2
    assert len(fromAssets) == 2
    assert environment.notional.signedBalanceOf(accounts[0], erc1155ids[0]) == 100e8
    assert environment.notional.signedBalanceOf(accounts[0], erc1155ids[1]) == 50e8
    assert environment.notional.signedBalanceOf(accounts[1], erc1155ids[0]) == 100e8
    assert environment.notional.signedBalanceOf(accounts[1], erc1155ids[1]) == 150e8
